...
score_matrix_2 = np.loadtxt(sys.argv[2], dtype=float) # matrix containing similartity scores of second model
...
# Take nearly half threshold points (temporary)
Threshold_weight = Threshold_weight[::5]
#Threshold_weight = Threshold_weight[::3]
...
plt.plot(false_positive_rate,true_positive_rate, label=sys.argv[1])
#Random_indices = np.sort(np.random.choice(len(true_positive_rate), 500, replace=False))
#fpr_new, tpr_new = [], []
#for i in Random_indices:
#	fpr_new.append(false_positive_rate[i])
#	tpr_new.append(true_positive_rate[i])
#plt.plot(fpr_new,tpr_new, 'ro', label=sys.argv[1] + ' individual points')
...
for i in range(7771):                                                                                        
    ...
    for j in range(z,7771):                                                                                  
        if masked_matrix[i][j] == 1:                                                                         
            Genuine_score_2.append(score_matrix_2[i][j])                                                         
        elif masked_matrix[i][j] == 2:                                                                       
            Imposter_score_2.append(score_matrix_2[i][j])                                                        
...
for i in range(len(Threshold_weight)):
    ...
    fp_nfp = 0
    threshold_value = Threshold_weight[i]
    for z in range(len(Genuine_score_2)):
        if Genuine_score_2[z] >= threshold_value:
            True_positive = True_positive+1
        # Extra analysis
        if Genuine_score[z] >= threshold_value:
            if Genuine_score_2[z] >= threshold_value:
                ...
                tp_ntp += 1
    for z1 in range(len(Imposter_score_2)):
        if Imposter_score_2[z1] >= threshold_value:
            False_positive=False_positive+1
        # Extra analysis
        if Imposter_score[z1] >= threshold_value:
            if Imposter_score_2[z1] >= threshold_value:
                ...
                fp_nfp += 1
    roc_based_changes.append([tp_tp/len(Genuine_score), tp_ntp/len(Genuine_score), fp_fp/len(Imposter_score), fp_nfp/len(Imposter_score)])
    False_PR=False_positive/len(Imposter_score)
    false_positive_rate_2.append(False_PR)
    True_PR=True_positive/len(Genuine_score)
    true_positive_rate_2.append(True_PR)
print ('True_positive_rate and False_positive_rate generated')


np.savetxt('old_vs_new.txt', roc_based_changes)
# plotting second 1000 point ROC
plt.plot(false_positive_rate_2,true_positive_rate_2, label=sys.argv[2])
plt.xscale('log')
plt.legend()
fig1 = plt.gcf()
#plt.show()
...
fig1.savefig(sys.argv[3], dpi=100) # change the name by which to save figure accordingly to your requirement
